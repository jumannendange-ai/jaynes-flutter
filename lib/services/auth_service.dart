...
class AuthService {
    ...
    try {
      ...
      try {
        await _sb.from('profiles').upsert({
          ...
          'plan': 'trial', 'trial_end': trialEnd,
        });
      } catch (_) {}
      final token = res.session?.accessToken ?? '';
      await _save(token, uid, email, name, 'trial', trialEnd, '');
      return {'success': true, 'user': _map(uid, email, name, 'trial', trialEnd, '')};
    } on AuthException catch (e) {
      String msg = e.message;
      if (msg.toLowerCase().contains('already')) msg = 'Email hii tayari imesajiliwa.';
  ...
  static Future<Map<String, dynamic>> login(String email, String password) async {
    try {
      ...
      Map? profile;
      try {
        profile = await _sb.from('profiles').select('full_name,plan,trial_end,sub_end').eq('id', uid).maybeSingle();
      } catch (_) {}
      ...
      return {'success': true, 'user': _map(uid, email, name, plan, trialEnd, subEnd)};
    } on AuthException {
  ...
  static Future<Map<String, dynamic>?> getSession() async {
    ...
    return {
      ...
      'trial_end': p.getString('trial_end') ?? '',
      'sub_end':   p.getString('sub_end') ?? '',
    };
  }
  ...
  static bool isPremium(Map<String, dynamic> user) {
    ...
    if (plan == 'premium' && subEnd.isNotEmpty) {
      return DateTime.tryParse(subEnd)?.isAfter(DateTime.now()) ?? false;
    }
    if (plan == 'trial' && trialEnd.isNotEmpty) {
      return DateTime.tryParse(trialEnd)?.isAfter(DateTime.now()) ?? false;
    }
    return false;
  }

  static Future<void> _save(String token, String uid, String email,
      String name, String plan, String trialEnd, String subEnd) async {
    final p = await SharedPreferences.getInstance();
  ...
  static Map<String, dynamic> _map(String uid, String email, String name,
      String plan, String trialEnd, String subEnd) =>
    {'uid': uid, 'email': email, 'name': name, 'plan': plan, 'trial_end': trialEnd, 'sub_end': subEnd};
}
